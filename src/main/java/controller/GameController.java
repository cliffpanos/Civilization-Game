...
public class GameController {
    ...
    public static void setMapDimensions(int dim1, int dim2) {
        rows = dim1;
        columns = dim2;
    }
    ...
    private static boolean move(TerrainTile start, TerrainTile end) {
        if (!(end.isEmpty() && GridFX.adjacent(end, start)
            ...
            if (!(start.getOccupant().getOwner() == null)
                && !(start.getOccupant().getOwner() instanceof Bandit)) {
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setHeaderText("This unit was not able to be\nmoved "
                    + "to the selected tile.");
                alert.setTitle("Unit Could Not Move!");
                alert.showAndWait();
    ...
    private static void attackTile(TerrainTile attacker, TerrainTile enemy) {
        if (enemy.isEmpty()
            ...
            state = GameState.NEUTRAL;
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText("This unit was not able to attack.");
            alert.setTitle("Unit Could Not Attack!");
            alert.showAndWait();
            return;
        }
    ...
    private static boolean nearSettlement(TerrainTileFX tile) {
        ...
        //return tile.getTile().isEmpty();
        int row = tile.getTile().getRow();
        int col = tile.getTile().getCol();
        Map map = GridFX.getMap();
        for (int r = row - 1; r < row + 2; r++) {
            for (int c = col - 1; c < col + 2; c++) {
                if (c >= 0 && c < tile.getTile().getCol() + 2
                    && r >= 0 && r < tile.getTile().getRow() + 2
                    && r < map.getRows() && c < map.getColumns()) {
