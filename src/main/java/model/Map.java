...
public class Map {
    ...
    //this will also populate the map even when rows =/= columns
    private void populateMap() {

        double scale = (((double) columns + (rows * 0.5)) / 17);
        ...
        for (int r = 0; r < rows; r++) {
            ...
            for (int c = 0; c < columns; c++) {
                ...
                if (value > (8 * scale) && value < (14 * scale)
                    && c < (5 * scale) && r > (5 * scale)) {
                    type = TileType.DESERT;
                } else if (value > (10 * scale) && value < (16 * scale)) {
                    type = TileType.FOREST;
                }
                if (value > (14 * scale) && value < (18 * scale)) {
                    type = TileType.MOUNTAIN;
                } else if (value >= (18 * scale)) {
                    type = TileType.ICE;
                }
    ...
    public static void putSettlement(String name, Civilization civ,
                                     int r, int c) {
        Settlement settlement = civ.getSettlement(name);
        map[r][c].setOccupant(settlement);
    ...
    public static void addEnemies(Bandit bandit, int numEnemies) {
        for (int i = 0; i < numEnemies; i++) {
            ...
            putSettlement("Bandit Hideout", bandit, rows - 1, 0);
            while (!map[r][c].isEmpty()) {
                //try to get new coords
                r = new Random().nextInt(rows);
                c = new Random().nextInt(columns);
            }
            ...
            if (!current.isEmpty()
                ...
                for (int dr = current.getRow() - 1; dr <= current.getRow() + 1;
                    ...
                    for (int dc = current.getCol() - 1;
                        ...
                        if  (dr >= 0 && dr < rows && dc >= 0 && dc < columns) {
